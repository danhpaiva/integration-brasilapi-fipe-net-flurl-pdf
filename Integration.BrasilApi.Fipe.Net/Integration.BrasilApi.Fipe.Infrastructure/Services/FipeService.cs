...
public class FipeService : IFipeService
{
    ...
    public FipeService(IConfiguration configuration)
    {
        _baseUrl = configuration.GetValue<string>("BrasilApiConfig:BaseUrl")
                   ?? "https://brasilapi.com.br/api/fipe/preco/v1/";
    }

    public async Task<VeiculoFipeResponse?> ObterPrecoAsync(string codigoFipe)
    {
        // Agora utiliza a _baseUrl injetada
        var url = $"{_baseUrl}{codigoFipe}";
